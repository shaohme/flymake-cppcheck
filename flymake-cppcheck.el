...
(defun flymake-cppcheck--try-create-build-root ()
  (let ((project-root-dir (project-root (project-current))))
    (when project-root-dir
      (let ((cppcheck-buildir (expand-file-name ".cppcheck-build" project-root-dir)))
        (if (not (file-directory-p cppcheck-buildir))
          (make-directory cppcheck-buildir))
        cppcheck-buildir))))
...
(defun flymake-cppcheck (report-fn &rest _args)
  ...
  (let* ((output (shell-command-to-string (concat flymake-cppcheck-program " --version")))
    ...
    (let* ((source (current-buffer))
           ...
           (cppcheck-builddir (flymake-cppcheck--try-create-build-root)))
      ...
      (let ((project-root-dir (project-root (project-current))))
        (when project-root-dir
          (let ((cppcheck-includes-file (expand-file-name ".cppcheck-includes" project-root-dir))
                (cppcheck-suppression-file (expand-file-name ".cppcheck-suppressions" project-root-dir)))
            ;; add suppressions and include lists if respective files exists
            (if (file-exists-p cppcheck-suppression-file)
                (push (format "--suppressions-list=%s" cppcheck-suppression-file) cppcheck-args ))
            (if (file-exists-p cppcheck-includes-file)
                (push (format "--includes-file=%s" cppcheck-includes-file) cppcheck-args )))))
      ...
      (when cppcheck-builddir
        (push (format "--cppcheck-build-dir=%s" cppcheck-builddir) cppcheck-args))
